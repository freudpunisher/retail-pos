    ...
    try {
        ...
        const barSales = await db
            ...
            .where(and(
                ...
                eq(products.productType, "drink"),
            ))

        const revenue = Number(barSales[0]?.revenue || 0)
        const cogs = Number(barSales[0]?.cogs || 0)
        const grossProfit = revenue - cogs
        const margin = revenue > 0 ? (grossProfit / revenue) * 100 : 0
        const totalQuantity = Number(barSales[0]?.totalQuantity || 0)
        const transactionCount = Number(barSales[0]?.transactionCount || 0)
        ...
        console.error("Failed to fetch bar finance data:", error)
        return NextResponse.json({ error: "Failed to fetch bar finance data" }, { status: 500 })
    }
}
